 ...
 * file DFRobot_PH.cpp
 *
 * Arduino library for a Cheap Analog pH Sensor, derriven from the work that DFRobot did for their Gravity: Analog pH Sensor / Meter Kit V2, SKU: SEN0161-V2
 *
...
DFRobot_PH::DFRobot_PH()
{
    ...
    this->_phValue        = 7.0;
    this->_acidVoltage    = 3032.44;    //buffer solution 4.0 at 25C
    this->_neutralVoltage = 2500.0;     //buffer solution 7.0 at 25C
    this->_voltage        = 2500.0;
}
...
void DFRobot_PH::begin()
{
    ...
    if(EEPROM.read(PHVALUEADDR)==0xFF && EEPROM.read(PHVALUEADDR+1)==0xFF && EEPROM.read(PHVALUEADDR+2)==0xFF && EEPROM.read(PHVALUEADDR+3)==0xFF){
        this->_neutralVoltage = 2500.0;  // new EEPROM, write typical voltage
    ...
    if(EEPROM.read(PHVALUEADDR+4)==0xFF && EEPROM.read(PHVALUEADDR+5)==0xFF && EEPROM.read(PHVALUEADDR+6)==0xFF && EEPROM.read(PHVALUEADDR+7)==0xFF){
        this->_acidVoltage = 3032.44;  // new EEPROM, write typical voltage
...
{
    float slope = (7.0-4.0)/((this->_neutralVoltage-2500.0)/5.0 - (this->_acidVoltage-2500.0)/5.0);  // two point: (_neutralVoltage,7.0),(_acidVoltage,4.0)
    float intercept =  7.0 - slope*(this->_neutralVoltage-2500.0)/5.0;
    //Serial.print("slope:");
    //Serial.print(slope);
    //Serial.print(",intercept:");
    //Serial.println(intercept);
    this->_phValue = slope*(voltage-2500.0)/5.0+intercept;  //y = k*x + b
    return _phValue;
}
...
void DFRobot_PH::phCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        case 2:
        if(enterCalibrationFlag){
            if((this->_voltage>2322)&&(this->_voltage<2678)){        // buffer solution:7.0{
                Serial.println();
                Serial.print(F(">>>Buffer Solution:7.0"));
                this->_neutralVoltage =  this->_voltage;
                //this->_neutralVoltage = 2500.0; 
				Serial.println(F(",Send EXITPH to Save and Exit<<<"));
                Serial.println();
                phCalibrationFinish = 1;
            }else if((this->_voltage>2679)&&(this->_voltage<3310)){  //buffer solution:4.0
                Serial.println();
        ...
        if(enterCalibrationFlag){
            ...
            if(phCalibrationFinish){
                if((this->_voltage>2322)&&(this->_voltage<2678)){
                    EEPROM_write(PHVALUEADDR, this->_neutralVoltage);
                }else if((this->_voltage>2679)&&(this->_voltage<3310)){
                    EEPROM_write(PHVALUEADDR+4, this->_acidVoltage);
                }
